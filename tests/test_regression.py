...
class TestReconstructionMetricsRegression:
    ...
    def test_reference_values_stability(self, reference_metadata):
        ...
        assert abs(stats["fc_correlation"]["mean"] - expected["fc_correlation_mean"]) / expected["fc_correlation_mean"] < rtol
        assert abs(stats["riemannian_distance"]["mean"] - expected["riemannian_distance_mean"]) / expected["riemannian_distance_mean"] < rtol
