...
def plot_fc_comparison(original: np.ndarray, reconstructed: np.ndarray, 
                       results: ReconstructionMetrics, save_path: str = None):
...
def plot_timeseries_comparison(original: np.ndarray, reconstructed: np.ndarray,
                                mask: np.ndarray, name: str, save_path: str):
...
def run_evaluation(data_dir: str, deterministic: bool = False, seed: int = 42, 
                   max_subjects: int = 5, model_size: str = "650M"):
    ...
    print(f"Mode: {'DETERMINISTIC (seed=' + str(seed) + ')' if deterministic else 'RANDOM MASKING'}")
    ...
    if len(all_results) > 0:
        ...
        for field in metric_fields:
            ...
            print(f"{field:20s}: {stats['mean']:8.4f} +/- {stats['std']:6.4f}  "
        ...
        with open(output_dir / "README.txt", 'w') as f:
            ...
            f.write(f"Masking mode: {'Deterministic (seed=' + str(seed) + ')' if deterministic else 'Random'}\n")
            f.write("\nFiles:\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate BrainLM reconstruction quality"
    )
    parser.add_argument(
        "--data-dir", "-d",
    ...
    parser.add_argument(
        "--deterministic",
        action="store_true",
        help="Use deterministic masking"
    )
    parser.add_argument(
        "--seed",
        type=int,
