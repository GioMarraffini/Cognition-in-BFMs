...
def match_embeddings_to_scores(subjects: np.ndarray, embeddings: np.ndarray, 
...
def train_predictor(X_train: np.ndarray, y_train: np.ndarray,
                    ...
                    model_type: str = "ridge", pca_dim: int = None) -> dict:
...
def main():
    parser = argparse.ArgumentParser(description="Train cognition predictor from BrainLM embeddings")
    parser.add_argument("--data-dir", "-d", default="data/aomic_cognition", 
                        help="Path to data directory with embeddings.npz")
    parser.add_argument("--predictor", "-p", default="ridge", choices=["ridge", "linear", "mlp"],
                        help="Predictor type")
    parser.add_argument("--pca-dim", type=int, default=None, 
                        help="PCA dimensionality reduction (e.g., 50)")
    ...
    results = train_predictor(X_train, y_train, X_test, y_test, 
                              model_type=args.predictor, pca_dim=args.pca_dim)
