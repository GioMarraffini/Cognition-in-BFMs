...
def analyze_metrics(csv_file: str):
    ...
    for metric in metrics:
        ...
        print(
            f"{metric:20s}: mean={mean_val:8.4f}  std={std_val:6.4f}  "
    ...
    ax4.set_title(
        f"FC Correlation vs Riemannian Distance\nr={r:.3f}, p={p:.4f}",
        fontsize=11,
        fontweight="bold",
    )
    ax4.grid(alpha=0.3)
    ...
    ax5.axvline(
        mean_rd + std_rd, color="orange", linestyle="--", linewidth=1, alpha=0.7, label="±1 std"
    )
