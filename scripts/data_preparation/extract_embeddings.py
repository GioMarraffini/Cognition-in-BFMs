...
def main():
    parser = argparse.ArgumentParser(description="Extract BrainLM embeddings from preprocessed fMRI")
    parser.add_argument("--data-dir", "-d", default="data/aomic_cognition", 
                        help="Path to data directory with processed/{train,test}/ subdirs")
    parser.add_argument("--model-size", "-m", default="650M", choices=["97M", "650M"], 
    ...
    parser.add_argument("--output", "-o", default=None, 
                        help="Output file path (default: <data-dir>/embeddings.npz)")
    ...
    np.savez(
        ...
        test_embeddings=np.array(list(test_embeddings.values())) if test_embeddings else np.array([]),
    )
    ...
    print(f"Test embeddings: {len(test_embeddings)} subjects x {list(test_embeddings.values())[0].shape[0] if test_embeddings else 0} dims")
