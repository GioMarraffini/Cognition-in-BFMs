        ...
        if sub_dir.is_dir():
            if use_mni:
                ...
                fmri_path = (
                    ...
                    / sub_dir.name
                    / "func"
                    / f"{sub_dir.name}_task-moviewatching_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz"
                )
            else:
                # Check for raw data
...
def prepare_cognition_data(
    ...
    train_df, test_df = train_test_split(
        df_valid, test_size=test_size, random_state=random_seed, shuffle=True
    )
...
def main():
    ...
    parser.add_argument(
        "--data-dir", "-d", default="data/openneuro_cog/ds003097", help="Path to AOMIC dataset"
    )
    parser.add_argument(
        "--output-dir", "-o", default="data/aomic_cognition", help="Output directory"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--n-train", type=int, default=5, help="Number of training subjects to download fMRI for"
    )
    parser.add_argument(
        "--n-test", type=int, default=5, help="Number of test subjects to download fMRI for"
    )
    parser.add_argument(
        "--use-raw", action="store_true", help="Use raw fMRI instead of MNI-preprocessed"
    )
