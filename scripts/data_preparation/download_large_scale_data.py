...
def download_development(output_dir="data/nilearn"):
    ...
    data = datasets.fetch_development_fmri(n_subjects=None, data_dir=output_dir)
...
OPENNEURO_DATASETS = {
    "aomic-id1000": {
        ...
        "cognitive_vars": [
            ...
            "BAS_drive",
            "BIS",  # Behavioral inhibition/activation
        ],
...
def download_openneuro(
    dataset_key: str,
    output_dir: str = "data/openneuro_cog",
    n_subjects: int = None,
    ...
    if participants_file.exists():
        ...
        cog_cols = [
            ...
            if any(
                kw in c.lower()
                for kw in ["ist_", "neo_", "stai", "bas", "bis", "iq", "memory", "fluid"]
            )
        ]
        if cog_cols:
...
Examples:
    ...
    if args.dataset in OPENNEURO_DATASETS:
        ...
        download_openneuro(
            args.dataset, output_dir, args.n_subjects, include_derivatives=not args.no_derivatives
        )
        print()
