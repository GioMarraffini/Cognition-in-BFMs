...
def organize_features(features_dict: dict, include_patches: bool) -> dict:
    ...
    if include_patches and "patch_embeddings" in features_dict[subjects[0]]:
        result["patch_embeddings"] = np.array([features_dict[s]["patch_embeddings"] for s in subjects])
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Brain-JEPA features with batch inference"
    )
    parser.add_argument(
        "--data-dir", "-d", default="data/aomic_cognition",
