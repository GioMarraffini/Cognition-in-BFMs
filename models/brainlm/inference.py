...
def load_model(
    ...
    config = ViTMAEConfig.from_pretrained("vandijklab/brainlm", subfolder=f"vitmae_{size}")
    config.update({"mask_ratio": mask_ratio, "output_attentions": output_attentions})

    model = (
        ViTMAEForPreTraining.from_pretrained(
    ...
    print(
        f"✓ BrainLM {size} loaded ({sum(p.numel() for p in model.parameters()) / 1e6:.0f}M params)"
    )
...
def extract_embeddings_batch(
    ...
    embeddings = {}
    iterator = (
        tqdm(npy_files.items(), desc="Extracting embeddings") if verbose else npy_files.items()
    )
